...
Get-ADObject -Filter 'objectcategory -eq "cn=site-link,cn=schema,cn=configuration,dc=domain,dc=com"' -SearchBase 'cn=configuration,dc=domain,dc=com' -Properties options | Set-ADObject -Replace @{options=$($_.options -bor 1)} 
